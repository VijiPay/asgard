import { IUserRepository } from '#interface/i_user_repository';
import User from '#models/user';
import { DateTime } from 'luxon'

export class UserRepository implements IUserRepository {
  async create(data: Partial<User>): Promise<User> {
    const { email, password, firstName, lastName, countryCode, googleId, facebookId, loginIp } = data
    console.log(data)
  ...
  async lockUser(id: number, reason: string, lockingUserId: number): Promise<User> {
    const user = await User.findOrFail(id)
