...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<User> {
    ...
    if (user.password === null) {
      const provider = user.googleId ? 'Google' : user.facebookId ? 'Facebook' : 'Google / Facebook'
      throw new CustomException(`Please login with ${provider} or reset your password`, 403)
    }
  ...
  async resetPassword(token: string, newPassword: string): Promise<User> {
    ...
    if (!user || user.passwordResetExpires! < DateTime.now()) {
  ...
  async verifyEmail(token: string): Promise<User> {
    ...
    if (!user || user.emailVerifyExpires! < DateTime.now()) {
      throw new CustomException('Invalid or expired verification token', 401)
    }
  ...
  async oauthLogin(provider: 'google' | 'facebook', profile: any): Promise<User> {
